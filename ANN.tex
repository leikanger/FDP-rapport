...
We will see that the neuron has a boolean carracteristic, in the form that the neuron as a whole either sends output through all its output synapses of rests. %TODO Sjekk. Slutten er skrevet på natta(feil ende: 0700). Før gjald setn over.
...
In the LIF neuron model, the node's value is computed as a leaky integration.%  with respect to time. %%%, an intregration that is leaky with respect to time.
All the input to the node is intragrated over time with a leakage propotional to the value of the node. This happens every time step.
