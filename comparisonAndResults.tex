...
%XXX HOVEDFORSKJELL: SN : integrerer input for å få state. 		KN: Umiddelbart input gir state, sammen med input historie.
	...
	In this section, the differences in design for the individual elements of the two model--specific nodes are presented. %TODO Dårlig setning!
	...
	%The elements are compared by the function that defines the element's behavior in time, the \emph{timeInterface} derived virtual function \emph{doTask()}.
	...
	% %The results for the difference in dendritic integration of all input synapses transmission will then be presented.
		...
		%If a delay is modeled into the simulation, the action potential is initiated after $n$ time steps, were $n$ is the predefined delay.
...
\label{secComparisonOfMechanismsOfNodeElements}
	...
	\subsection{Time Course of Nodes' Depolarization Value}
...
\end{figure}
	...
	%		 	as the the node never will fire for a sub--threshold activity variable. 
		...
		%	As the synaptic transmission is defined as the synaptic weight times the inverse of the period, the synaptic transmission is zero when $\kappa$ is less than $\tau$.
			...
			%For the edge transmission we therefore see the strange behavior in fig. \ref{figEdgeTransmission:edge-b} when the activity variable becomes less than or equal to the threshold. %XXX Skriv om. 
