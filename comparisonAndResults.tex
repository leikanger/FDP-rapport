...
%XXX HOVEDFORSKJELL: SN : integrerer input for å få state. 		KN: Umiddelbart input gir state, sammen med input historie.
	...
	% %The results for the difference in dendritic integration of all input synapses transmission will then be presented.
		...
		From sec. \ref{ssecTheSynapse}, we can se that this gives a direct simulation of synaptic transmission in the the biological synapse.
...
\begin{figure}[hbt!]
	...
	\caption{The depolarization curve for a SANN node and a $\kappa$ANN node with the same input. The sensor function has an input equivalent to an activation level of $\kappa = 1.5 \tau$ for both nodes. 
			(Plot generated by AuroSim)} %XXX Eller for begge nodeModeller
...
\begin{figure}[hb!tp]
	...
	\caption{Synaptic transission for a synapse with a presynaptic sensor funciton \mbox{$f(t) = 1.1 \cdot \tau \cdot (1 + sin( 2\pi \cdot \frac{t}{1000})$}. 
			Notice that the sensor fuction allways will be larger than the firing threshold. 	(Plot generated by AuroSim)} %TODO skriv om dette tomrommet i teksten under.
...
%%XX XXX XXX XXX XXX XXXXXXXXXXXXXXXXXXXXXXXXXXX XXX XXXXXXXXXXXXXXXXXXXXXX XXX XXXXXXXXXX
