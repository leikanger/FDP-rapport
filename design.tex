...
\section{Design} % eller "Design; General Consepts for both Imlementations" eller noke anna..
	...
	The qualitative differences between the ANN model based on a direct simulation of the neuron and the developed model will be presented in this and the next chapter.

	% XXX Skriv om starten!
	A neural simulator is placed in an environment, and will have a general design of each node. In the remainder of this text this will be referred to as the framework of the simulation.
	
	For a comparison between the two models, both models have been designed and implemented.
	To optimize the implementations for comparison between the two models, the most appropriate solution is to use a common framework for the two simulators.
	This will make the differences and similarities between the two designs/implementations more prominent.
	...
	This section will start with describing the common framework of the implementations; 
		General concepts such as simulated asynchronicity (sec.\ref{ssecTime}), generality of the implementation and the design of the indivitual node will be presented in this part. %TODO sett på internreferanser til rett subsection.
	...
	I will in the remainder of this text refer to this as simulated asynchronicity.
	%To get the same degree of asynchronicity in simulated neurons, time will also have to be simulated.

	In digital electronics, asynchronous design refers to clockless circuits. Every part of the circuit executes its part independently of the other, in respect to time.
	...
	The concept of simulated time therefore becomes wery important for the simulator. 

	The intuitive solution to the concept of simulation of time is to iterate through all nodes for each time step, and update the value as a consequence of time propagation.
	As efficiancy is important for a neural simulator that is to be used in technology, it is undesirable to do the calculation for each node every time iteration.
	...
	This is based on a linked list with the different task elements, and a scheduler function that calls a certain function for each element.
	If each element is an object of a class derived of a common interface class, the scheduler function could call one of the member functions of this interface class.
	...
	Here, the abstract time interface class is called \emph{class time\_interface}.
	The class definition can be seen in appendix \ref{appendixTimeInterfaceDefinition}.
	...
	The function that is responsible for execution of the task of each object in \emph{pWorkTaskQue} is the virtual function \emph{doTask()}, derived from \emph{timeInterface}.
	This function lists all actions that are to be performed in a  
	The function doCalculation() will become important later, when we discuss synaptic transmission for the new model in sec. \ref{ssecImpOfSynTransmissionKANNN}.

	The interface class contains two pure virtual functions and one variable, all important for the derived element's interraction with time.  %XXX Tja.. Skriv litt om..
	
	It contains the functions \emph{doTask()} and \emph{doCalculation()}.





	% TODO Gå rett på ideen min, og seinere forklar det med to lister (Som Koch seinare forklarte meg om).
	%This could be done by having two lists %eller bare gå rett på ideen min, og seinere forklare med to lister?

	It is better to use a scheme based on serial execution, where the last element of the time iteration's task list is responsible for iteration of time.
	In the case of to large a work load, the iteration of time will be slowed down as well as the execution of tasks.
	%In this scheme, the work load of the system will slow down the iteration of time as well as the execution of tasks. 
	As real world time is unimportant for the simulated system, this serial scheme for time simulation will be the choice of this implementation.
	%As this is much less error prone for the simulated system, this will be the choise for this implementation. %affect the time it will take to iterate the simulated time.

	Serial time simulation could be implemented by having one task list for each new time iteration. 
	For this implementation, an alternative approach with only one list has been implemented.
	Here all new tasks are appended last in a linked list. A separate tasks is responsible to iterate time, and appoint a new time separation task at the end of the list.
	If the list consists of pointers, the task could add a pointer to itself instead of creating a new object for insertion.

	In this implementation, a linked list is declared static in class \emph{time\_class}, meaning that only one instance of the variable is created for all objects of class \emph{time\_class}.
	This list is named \emph{pWorkTaskQue}, and is the list where the elements' \emph{doTask()} function is called by the scheduler function.
	%and is the list executed by the sceduler function.

	% This can be done by having a linked list of tasks planned for execution, \emph{pWorkTaskQue}. This is defined as a static variable of class \emph{time\_class} (see appendix \ref{appendixTimeClass}).
	% When one task is completed, a scheduler function commence the execution of the next task on the list. % XXX feil ord? commence
...
%XXX Trenger å koble sammen med videre tekst.	
	...
	%A derived class also inherit the pure virtual functions.
	%Every class with a pure virtual function becomes is an abstract class, and no object of that class can be created\cite{Stroustrup2000KAP12}.


	%The list that contains future tasks is a list of pointers to the \emph{timeInterface} class, so only classes derived from \emph{timeInterface} is scheduled by the simulated time dimention.
	%Because the member function \emph{doTask()} is defined as a pure virtual function in timeInterface (se appendix \ref{appendixTimeClassDefinition}), no instance can be made of it.

	Let the list be of type \emph{std::list$<$father*$>$ LIST} and \emph{$[father]$} contain pure virtual fuctions.
		, then only pointers to objects derived from \emph{[father]} can be placed in the list. 
	Further, all derived classes will inherit the pure virtual function, and thus be unable to make an instance of unless the fuction is defined in the derived class.

	This gives that every object used in \emph{pWorkTaskQue} and thus the time scheduler, has its own defined \emph{doTask()} function.
	As this is the function that is called by the scheduler function, every object of a class derived from \emph{class timeInterface} have a defined behavior connected to the simulated time.

	As we can see, the \emph{pWorkTaskQue} is a list of pointers to elements of the \emph{timeInterface} class, meaning that only pointers to instances of \emph{class timeInterface} or derived classes can be inserted into the list.


	The timeInterface derived class that is most important to the simulated asynchronisity is the class \emph{time\_class}.

	As seen in appendix \ref{appendixTimeClassDoTask}


% XXX ------------------- XXX --------------- xxx-------------------------xxx--------------------------xxx
\newpage

%	This will create a stong dependence on the work load of the simulation, and will be error prone.

	%The concept of simpulated asynchronicity, 
