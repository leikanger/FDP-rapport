...
In terms from neuroscience this meens that the network og neurons is recurrent (with feedback) and that the synapses (the connections between neurons) are directional (information flows in one direction). 
...
% eller: %In one synapse you typically have a combination of neurotransmitter release and postsynaptic receptors that causes the whole synapse to either be exitatory or inhibitory.
...
This establishes the self carrying signal of the axon, and results in a similar depolarization for the different axon terminals\cite{PrinciplesOfNeuralScience4edKAP09}.
...
As the electrical potential spead passively to the next site with voltage--gated channels, this is impotant to hinder the action potential going ``the wrong way''.
...
The axon is organized as a tree, with one trunk close to the soma of the neuron called the axon hillock, and branches called axon collaterals. 
