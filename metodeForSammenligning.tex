...
%DENNE er en god forklaring. men det er feil veil (s er før/over K)
\subsubsection{Time responce of the neuron}
The analysis in the previous section is sound, but if we isolate the error by giving the sensor aurons a constant sensor function with an activity level of $\kappa = 1.5 \tau$ we se that the error is oposite of the predicted error.
In the analysis in the previous section, the SN would have an error that gives a value that is higher than it should be.
This implies that the depolarization curve of the SN should be ``before'' that of the $\kappa$N.
In figure \ref{figComparisonBetweenSsensorAndKsensorDepolCurveCONStActivityLevel} we se that the error gives that the SN is "after".

%it predicts the opposite error than the diffenerce between the $\kappa$N and the SN in fig. \ref{figComparisonBetweenSsensorAndKsensorDepolCurve}.
According to the analysis, the depolarization of the SN should be more than (or ``before'') the depolarization kurve for the $\kappa$N.
This does not mean that the analysis is incorrect, only that it is overshadowed by some other, more important error.

A plausible canditate is that this error is comes from using a floating point variable. 
Floating point variables has a varying degree of %nøyaktighet. Fører til avrundingsfeil ved kvar operasjon => SANN får tidligere fyring ?
% Sjekka i octave. Begge er feil, men SANN er rettest. (14 fyringer skal ta 3845. KANN: 3838, SANN: 3864). Med refraction time

For example the variable $p(\kappa) = 274,65$ for $\kappa = 1.5 \tau$ can get rounded to 274.7

OK: eit problem er løstninga mi for refraction time for K\_auronet. Begynner å lade opp umiddelbart, og legger til en for estimering av fyring.
Dette vil skape problem om kappa endres etter start, men ikkje ellers..
Føkk.
...
\subsection{Floating point calculation pitfalls}
