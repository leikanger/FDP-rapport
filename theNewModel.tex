...
\chapter{Mathematical modelling of the biological neuron} 
...
\label{secTheEquilibriumPotential}
...
To solve this problem, the concept of ``a time window'' is introduced. A time window is defined as the longest time interval where the activation level of the node is constant.
Within each time window both \eqref{eqVerdiligninga} and \eqref{eqTidTilFyringVedEndraKappa} is therefore valid. % .. kan bli brukt.
