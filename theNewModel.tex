...
%XXX TODO Neste linjene er litt feil. Dette beskrive oppladninga av depol., m.a.o. ikkje heile p_isi(K). Bare p_charge,isi(K). Skriv dette. TODO
...
If $\kappa$ varies during one period, $t^*$ from equation \eqref{eqTidTilFyringVedEndraKappa} is an estimate of the remainting time until firing. This estimate is updated every time $\kappa$ is.
...
This value is equal to the value $v(t)$ at the time of change from the previous time window(se fig. \ref{figVerdifunksjonen}).
...
%\subsection{Values for the value equation}
...
Equation \ref{eqPeriodeligningForKonstIntraPeriodKAPPA} describes the depolarizing phase of the neuron.
...
\begin{equation}
	...
	%									 &= \frac{-\alpha}{\ln \left( \lim_{\kappa->\inf}\left( \frac{\kappa - \tau}{\kappa} \right) \right)- \alpha t_r } \\
	\end{split}
\end{equation}

Using l'Hôpital's rule %INNI ln(-) funksjonen. TODO Viktig å få med dette i denne setninga!
%on $\lim_{\kappa->\infty}(\ln \left( \frac{\kappa - \tau}{\kappa} \right)$
...
For SANN nodes, this means that the node will not change its value for the duration of the next time step. For $\kappa$ANN this can be implemented as a 
%TODO fullfør!
